<script setup></script>

<template>
  <div class="relative max-w-screen-xl mx-auto px-8 py-6">
    <header class="w-full flex justify-between items-center mb-10">
      <span class="font-extrabold sm:text-2xl text-sky-600">DS</span>
      <ul class="flex space-x-3 sm:space-x-10 text-sm sm:text-base">
        <li>About</li>
        <li>Projects</li>
        <li>Resume</li>
      </ul>
      <div class="flex space-x-3">
        <a class="text-blue-600" href="https://www.linkedin.com/in/mistershack/">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-current" width="24" height="24" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" /></svg>
        </a>
        <a href="https://github.com/MisterShack">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-4.466 19.59c-.405.078-.534-.171-.534-.384v-2.195c0-.747-.262-1.233-.55-1.481 1.782-.198 3.654-.875 3.654-3.947 0-.874-.312-1.588-.823-2.147.082-.202.356-1.016-.079-2.117 0 0-.671-.215-2.198.82-.64-.18-1.324-.267-2.004-.271-.68.003-1.364.091-2.003.269-1.528-1.035-2.2-.82-2.2-.82-.434 1.102-.16 1.915-.077 2.118-.512.56-.824 1.273-.824 2.147 0 3.064 1.867 3.751 3.645 3.954-.229.2-.436.552-.508 1.07-.457.204-1.614.557-2.328-.666 0 0-.423-.768-1.227-.825 0 0-.78-.01-.055.487 0 0 .525.246.889 1.17 0 0 .463 1.428 2.688.944v1.489c0 .211-.129.459-.528.385-3.18-1.057-5.472-4.056-5.472-7.59 0-4.419 3.582-8 8-8s8 3.581 8 8c0 3.533-2.289 6.531-5.466 7.59z" /></svg>
        </a>
      </div>
    </header>
    <main>
      <section class="py-32 sm:py-64">
        <div class="flex flex-wrap items-center justify-center item sm:space-x-10 relative">
          <div class="absolute w-32 sm:w-48 lg:w-64 border-b-4 border-sky-800 -top-10 left-10 md:left-20"></div>
          <div class="absolute w-32 sm:w-48 lg:w-64 border-b-4 border-sky-800 -bottom-10 right-2 md:right-12"></div>
            <div class="w-44 lg:w-56">
              <img src="https://media-exp1.licdn.com/dms/image/C4D03AQHDSvdWMdqkPA/profile-displayphoto-shrink_800_800/0/1607009628181?e=1644451200&v=beta&t=wEbQhp2rKCFbfvDR18yrAkVjbwFG9rvXRvMvgcgWMGQ" />
            </div>
          <div class="text-4xl md:text-6xl lg:text-8xl font-light mt-10 sm:mt-0">
            <div>Hello, I'm</div>
            <div class="font-bold text-sky-600">David Shack</div>
          </div>
        </div>
      </section>
      <section>
        <h1 class="text-center text-3xl mb-10">About Me</h1>
        <div class="leading-7 md:text-lg md:leading-10">
          <p class="my-5">I'm an enthusiastic Full-Stack Developer with an expanding interest in solving problems using efficient solutions. I learned HTML and CSS in middle school and over the years I have been building my development skills alongside my communication skills and professionalism.</p>
          <p class="my-5">After college I had the privilege to work for a daycare management software, Fastoche, where I strengthened my PHP, MySQL, JavaScript and CSS knowledge.</p>
          <p class="my-5">Currently, I am a Software Developer for Scootaround, a great MaaS (Mobility as a Service) company that offers mobility equipment rentals in over 2500 locations. Every day I apply my skills to help refine, refactor, and reform internal software to aid our day-to-day operation both frontend and backend. Scootaround has taught me how to effectively work and communicate within a team and stretch my critical thinking around business logic.</p>
          <p class="my-5">Throughout my developer journey I have learned a few things about being a developer:
            <ul class="text-sm md:text-base text-sky-900 font-semibold sm:pl-10">
              <li class="my-4 border-l-4 border-sky-600 pl-4">It's okay to take on challenges and own it, but know when to ask for help.</li>
              <li class="my-4 border-l-4 border-sky-600 pl-4">Stumbling across a page begging to be refactored doesn't mean you always have to fix it then. Make a //TODO and move on.</li>
              <li class="my-4 border-l-4 border-sky-600 pl-4">Take time to understand what and who you're developing for. Knowing business operations/logic can allow you to make wiser decisions for user experience, performance and scalability.</li>
              <li class=" my-4 border-l-4 border-sky-600 pl-4">Ask questions. Answer questions. Make suggestions. Listen and learn.</li>
            </ul>
          </p>
        </div>
      </section>
              <hr class="w-48 mx-auto border-sky-600 my-20" />

      <section>
        <h1 class="text-center text-3xl mb-10">Projects</h1>
        <div class="flex space-x-3 justify-center">
          <div class="p-5 rounded-lg w-full md:w-1/2 lg:w-1/3 bg-sky-100 shadow-md">
            <div class="flex justify-between">
              <h2 class="text-xl mb-3 text-sky-900 font-medium">Myze</h2>
              <div class="flex space-x-5">
                <a class="text-indigo-600" href="https://myze.ca">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                  </svg>
                </a>
                <a href="https://github.com/MisterShack/Myze-Client">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-4.466 19.59c-.405.078-.534-.171-.534-.384v-2.195c0-.747-.262-1.233-.55-1.481 1.782-.198 3.654-.875 3.654-3.947 0-.874-.312-1.588-.823-2.147.082-.202.356-1.016-.079-2.117 0 0-.671-.215-2.198.82-.64-.18-1.324-.267-2.004-.271-.68.003-1.364.091-2.003.269-1.528-1.035-2.2-.82-2.2-.82-.434 1.102-.16 1.915-.077 2.118-.512.56-.824 1.273-.824 2.147 0 3.064 1.867 3.751 3.645 3.954-.229.2-.436.552-.508 1.07-.457.204-1.614.557-2.328-.666 0 0-.423-.768-1.227-.825 0 0-.78-.01-.055.487 0 0 .525.246.889 1.17 0 0 .463 1.428 2.688.944v1.489c0 .211-.129.459-.528.385-3.18-1.057-5.472-4.056-5.472-7.59 0-4.419 3.582-8 8-8s8 3.581 8 8c0 3.533-2.289 6.531-5.466 7.59z" /></svg>
                </a>
              </div>
            </div>
            <p class="text-lg">A personal finance tracking app that allows you to manually manage your transactions and consolidate your bank/credit accounts.</p>
            <p class="mt-5 text-sm">Created using Vue.js, Tailwind CSS and Supabase</p>
          </div>
        </div>
      </section>

      <hr class="w-48 mx-auto border-sky-600 my-20" />

      <section>
        <h1 class="text-center text-3xl mb-20">Resume</h1>

        <div class="my-10">
          <h2 class="flex items-center text-2xl space-x-5 font-thin">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z" />
            </svg>
            <span>Skills</span>
          </h2>
          <ul class="pl-20 flex flex-col space-y-2 mt-5 list-disc list-inside">
          <li>Professional experience with PHP, JavaScript / jQuery, MySQL and CSS</li>
          <li>Knowledge of Vue.js, Tailwind CSS, Git, and Apache</li>
          <li>Writing readable, documented code with attention to detail and performance</li>
          <li>Ability to work in fast-pace environments</li>
          <li>Attentive to detail and minful of perfomance and efficiency</li>
          <li>Troubleshooting and resolving errors in a timely manner</li>
          </ul>
        </div>

        <div class="my-10">
          <h2 class="flex items-center text-2xl space-x-5 font-thin">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            <span>Professional Employment</span>
          </h2>
          <div class="pl-16">
            <div class="flex space-x-10 my-5 py-2 px-4 ">
              <div class="w-44">
                <div class="text-lg font-semibold">Software Developer</div>
                <a href="https://www.scootaround.com">Scootaround Inc.</a>
                <div class="text-gray-600">Jan. 2021 - Present</div>
              </div>
              <ul class="list-disc text-sm">
                <li class="my-2">Collaborate with internal and external teams to develop new features for clients and staff</li>
                <li class="my-2">Actively use GitHub for internal code reviews and managing pull-requests</li>
                <li class="my-2">Increased quote intake efficiency over 25% by re-developing internal booking page</li>
              </ul>
            </div>
              <hr />

            <div class="flex space-x-10 my-5 py-2 px-4 ">
              <div class="w-44">
                <div class="text-lg font-semibold">Lead Developer</div>
                <a href="https://www.fastoche.ca">Fastoche Canada</a>
                <div class="text-gray-600">Aug. 2016 - Dec. 2020</div>
              </div>
              <ul class="list-disc text-sm">
                <li class="my-2">Develop, design, test and maintain web applications for nearly 200 daycare centres across Canada with more than half in Manitoba</li>
                <li class="my-2">Developed a batch EFT system using Bambora's API to help process over $32,000 a month in transactions</li>
                <li class="my-2">Deployed a free electronic COVID-19 screening tool that improves efficiency, organization and reduce paper usage for over 250 daycare centres</li>
                <li class="my-2">Built a parent arrival estimator that allows daycares to better prepare children for pickup using Google Maps, Geocoding and Directions API</li>
                <li class="my-2">Create intricate systems such as rate calculation and subsidy estimation</li>
                <li class="my-2">Troubleshoot problems that arise from testing or through customer feedback and resolve in a timely manner</li>
                
              </ul>
            </div>

            <hr />
            <div class="flex space-x-10 my-5 py-2 px-4 ">
              <div class="w-44">
                <div class="text-lg font-semibold">Freelance Developer</div>
                <a href="https://www.fastoche.ca">Self-employed</a>
                <div class="text-gray-600">Feb. 2016 - Aug. 2016</div>
              </div>
              <ul class="list-disc text-sm">
                <li class="my-2">Design, develop and deploy website to DigitalOcean</li>
                <li class="my-2">Meet with clients to understand website requirements</li>
                <li class="my-2">Maintain client domains, DNS records and MySQL databases</li>
              </ul>
            </div>
          </div>
        </div>

        <div class="my-10">
          <h2 class="flex items-center text-2xl space-x-5  font-thin ">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path d="M12 14l9-5-9-5-9 5 9 5z" />
              <path d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z" />
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222" />
            </svg>
            <span>Education</span>
          </h2>
          <div class="pl-16">
            <div class="my-5 py-2 px-4">
              <div class="text-lg font-semibold">Business of Information Technology</div>
              <a href="https://www.rrc.ca/">Red River College</a>
              <div class="text-gray-600">March 2013 - February 2016</div>
            </div>
          </div>
        </div>

        <div class="my-10">
          <h2 class="flex items-center text-2xl space-x-5 font-thin">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
            </svg>
            <span>Actively Learning</span>
          </h2>
          <div class="pl-16 flex items-center mt-5">
            <a>
              <svg version="1.1" id="Layer_2_1_" class="h-12" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
                viewBox="0 0 841.9 595.3" enable-background="new 0 0 841.9 595.3" xml:space="preserve">
                <g>
                  <path fill="#61DAFB" d="M666.3,296.5c0-32.5-40.7-63.3-103.1-82.4c14.4-63.6,8-114.2-20.2-130.4c-6.5-3.8-14.1-5.6-22.4-5.6v22.3
                    c4.6,0,8.3,0.9,11.4,2.6c13.6,7.8,19.5,37.5,14.9,75.7c-1.1,9.4-2.9,19.3-5.1,29.4c-19.6-4.8-41-8.5-63.5-10.9
                    c-13.5-18.5-27.5-35.3-41.6-50c32.6-30.3,63.2-46.9,84-46.9l0-22.3c0,0,0,0,0,0c-27.5,0-63.5,19.6-99.9,53.6
                    c-36.4-33.8-72.4-53.2-99.9-53.2v22.3c20.7,0,51.4,16.5,84,46.6c-14,14.7-28,31.4-41.3,49.9c-22.6,2.4-44,6.1-63.6,11
                    c-2.3-10-4-19.7-5.2-29c-4.7-38.2,1.1-67.9,14.6-75.8c3-1.8,6.9-2.6,11.5-2.6l0-22.3c0,0,0,0,0,0c-8.4,0-16,1.8-22.6,5.6
                    c-28.1,16.2-34.4,66.7-19.9,130.1c-62.2,19.2-102.7,49.9-102.7,82.3c0,32.5,40.7,63.3,103.1,82.4c-14.4,63.6-8,114.2,20.2,130.4
                    c6.5,3.8,14.1,5.6,22.5,5.6c27.5,0,63.5-19.6,99.9-53.6c36.4,33.8,72.4,53.2,99.9,53.2c8.4,0,16-1.8,22.6-5.6
                    c28.1-16.2,34.4-66.7,19.9-130.1C625.8,359.7,666.3,328.9,666.3,296.5z M536.1,229.8c-3.7,12.9-8.3,26.2-13.5,39.5
                    c-4.1-8-8.4-16-13.1-24c-4.6-8-9.5-15.8-14.4-23.4C509.3,224,523,226.6,536.1,229.8z M490.3,336.3c-7.8,13.5-15.8,26.3-24.1,38.2
                    c-14.9,1.3-30,2-45.2,2c-15.1,0-30.2-0.7-45-1.9c-8.3-11.9-16.4-24.6-24.2-38c-7.6-13.1-14.5-26.4-20.8-39.8
                    c6.2-13.4,13.2-26.8,20.7-39.9c7.8-13.5,15.8-26.3,24.1-38.2c14.9-1.3,30-2,45.2-2c15.1,0,30.2,0.7,45,1.9
                    c8.3,11.9,16.4,24.6,24.2,38c7.6,13.1,14.5,26.4,20.8,39.8C504.7,309.8,497.8,323.2,490.3,336.3z M522.6,323.3
                    c5.4,13.4,10,26.8,13.8,39.8c-13.1,3.2-26.9,5.9-41.2,8c4.9-7.7,9.8-15.6,14.4-23.7C514.2,339.4,518.5,331.3,522.6,323.3z
                    M421.2,430c-9.3-9.6-18.6-20.3-27.8-32c9,0.4,18.2,0.7,27.5,0.7c9.4,0,18.7-0.2,27.8-0.7C439.7,409.7,430.4,420.4,421.2,430z
                    M346.8,371.1c-14.2-2.1-27.9-4.7-41-7.9c3.7-12.9,8.3-26.2,13.5-39.5c4.1,8,8.4,16,13.1,24C337.1,355.7,341.9,363.5,346.8,371.1z
                    M420.7,163c9.3,9.6,18.6,20.3,27.8,32c-9-0.4-18.2-0.7-27.5-0.7c-9.4,0-18.7,0.2-27.8,0.7C402.2,183.3,411.5,172.6,420.7,163z
                    M346.7,221.9c-4.9,7.7-9.8,15.6-14.4,23.7c-4.6,8-8.9,16-13,24c-5.4-13.4-10-26.8-13.8-39.8C318.6,226.7,332.4,224,346.7,221.9z
                    M256.2,347.1c-35.4-15.1-58.3-34.9-58.3-50.6c0-15.7,22.9-35.6,58.3-50.6c8.6-3.7,18-7,27.7-10.1c5.7,19.6,13.2,40,22.5,60.9
                    c-9.2,20.8-16.6,41.1-22.2,60.6C274.3,354.2,264.9,350.8,256.2,347.1z M310,490c-13.6-7.8-19.5-37.5-14.9-75.7
                    c1.1-9.4,2.9-19.3,5.1-29.4c19.6,4.8,41,8.5,63.5,10.9c13.5,18.5,27.5,35.3,41.6,50c-32.6,30.3-63.2,46.9-84,46.9
                    C316.8,492.6,313,491.7,310,490z M547.2,413.8c4.7,38.2-1.1,67.9-14.6,75.8c-3,1.8-6.9,2.6-11.5,2.6c-20.7,0-51.4-16.5-84-46.6
                    c14-14.7,28-31.4,41.3-49.9c22.6-2.4,44-6.1,63.6-11C544.3,394.8,546.1,404.5,547.2,413.8z M585.7,347.1c-8.6,3.7-18,7-27.7,10.1
                    c-5.7-19.6-13.2-40-22.5-60.9c9.2-20.8,16.6-41.1,22.2-60.6c9.9,3.1,19.3,6.5,28.1,10.2c35.4,15.1,58.3,34.9,58.3,50.6
                    C644,312.2,621.1,332.1,585.7,347.1z"/>
                  <polygon fill="#61DAFB" points="320.8,78.4 320.8,78.4 320.8,78.4 	"/>
                  <circle fill="#61DAFB" cx="420.9" cy="296.5" r="45.7"/>
                  <polygon fill="#61DAFB" points="520.5,78.1 520.5,78.1 520.5,78.1 	"/>
                </g>
              </svg>
            </a>
            <a class="ml-3">
              <svg class="h-12" fill="none" version="1.1" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" xmlns:cc="http://creativecommons.org/ns#" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
  <title>TypeScript logo</title>
  <rect width="512" height="512" rx="50" fill="#3178c6"/>
  <path d="m317 407v50c8.1 4.2 18 7.3 29 9.4s23 3.1 35 3.1c12 0 23-1.1 34-3.4 11-2.3 20-6.1 28-11 8.1-5.3 15-12 19-21s7.1-19 7.1-32c0-9.1-1.4-17-4.1-24s-6.6-13-12-18c-5.1-5.3-11-10-18-14s-15-8.2-24-12c-6.6-2.7-12-5.3-18-7.9-5.2-2.6-9.7-5.2-13-7.8-3.7-2.7-6.5-5.5-8.5-8.4-2-3-3-6.3-3-10 0-3.4 0.89-6.5 2.7-9.3s4.3-5.1 7.5-7.1c3.2-2 7.2-3.5 12-4.6 4.7-1.1 9.9-1.6 16-1.6 4.2 0 8.6 0.31 13 0.94 4.6 0.63 9.3 1.6 14 2.9 4.7 1.3 9.3 2.9 14 4.9 4.4 2 8.5 4.3 12 6.9v-47c-7.6-2.9-16-5.1-25-6.5s-19-2.1-31-2.1c-12 0-23 1.3-34 3.8s-20 6.5-28 12c-8.1 5.4-14 12-19 21-4.7 8.4-7 18-7 30 0 15 4.3 28 13 38 8.6 11 22 19 39 27 6.9 2.8 13 5.6 19 8.3s11 5.5 15 8.4c4.3 2.9 7.7 6.1 10 9.5 2.5 3.4 3.8 7.4 3.8 12 0 3.2-0.78 6.2-2.3 9s-3.9 5.2-7.1 7.2-7.1 3.6-12 4.8c-4.7 1.1-10 1.7-17 1.7-11 0-22-1.9-32-5.7-11-3.8-21-9.5-30-17zm-84-123h64v-41h-179v41h64v183h51z" clip-rule="evenodd" fill="#fff" fill-rule="evenodd" style="fill:#fff"/>
              </svg>
            </a>
            <a class="ml-5">
              <svg version="1.1" class="h-12" viewBox="0 0 261.76 226.69" xmlns="http://www.w3.org/2000/svg"><g transform="matrix(1.3333 0 0 -1.3333 -76.311 313.34)"><g transform="translate(178.06 235.01)"><path d="m0 0-22.669-39.264-22.669 39.264h-75.491l98.16-170.02 98.16 170.02z" fill="#41b883"/></g><g transform="translate(178.06 235.01)"><path d="m0 0-22.669-39.264-22.669 39.264h-36.227l58.896-102.01 58.896 102.01z" fill="#34495e"/></g></g></svg>
            </a>
            <a>
              <img class="h-16" src="https://repository-images.githubusercontent.com/130884470/ad63fd00-7ab2-11ea-9c57-6c114391183a">
            </a>

          </div>
        </div>

        
        <div class="my-10">
          <h2 class="flex items-center text-2xl space-x-5  font-thin">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z" />
            </svg>
            <span>Activities & Interests</span>
          </h2>
          <ul class="pl-20 flex flex-col space-y-2 mt-5 list-disc list-inside">
          <li>Volunteer A/V technician for A11yTO - Web Accessibility Conference</li>
          <li>Professional Development - Prairie Dev Con & ConFoo</li>
          <li>Reading up on the latest tech releases</li>
          <li>Camping in Manitoba</li>
          <li>Cooking & Baking</li>
          <li>Woodworking</li>
          </ul>
        </div>
      </section>

    </main>
  </div>
</template>
